...
무선통신사는 개인들의 이동전화를 통하여 통신데이터를 주고 받는데, 통신데이터에 기반한 유동인구데이터를 생산한다. 예를 들어, KT는 공공기관과 제휴하여 유동인구데이터를 파일 형태로 제공한다. 월기준으로 경기도 유동인구를 공간정보와 함께 제공하는데 대략 파일크기가 350 MB 가 넘는 크기를 보여주고 있다. 특히, `sex_age_22.csv` 파일은 2022년 경기도 전체
