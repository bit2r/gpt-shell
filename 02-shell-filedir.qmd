...
디렉토리는 *장소(place)* 같다 - 쉘을 사용할 때마다 정확하게 한 장소에 위치하게 되는데, 이를 **현재 작업 디렉토리(current working directory)**라고 부른다.
...
![파일 시스템](images/filesystem.svg){fig-align="center"}
...
![홈 디렉토리](images/home-directories.svg){fig-align="center"}
...
먼저, `data-shell` 디렉토리에 담긴 것을 살펴보자; 디렉토리 이름에 `ls`를 전달해서 앞서 확인된 동일한 전략을 사용하자:
...
상기 명령어는 홈 디렉토리에 바탕화면(`Desktop`) 디렉토리로 이동하고 나서, `data-shell` 디렉토리로 이동하고 나서, `data` 디렉토리에 이동하게 된다.
`cd` 명령어는 아무것도 출력하지는 않지만, `pwd` 명령어를 실행하게 되면 `/Users/nelle/Desktop/data-shell/data` 위치한 것을 확인하게 된다.
...
물론, `cd ..` 명령어를 실행하고 나서 `pwd`을 실행하게 되면 `/Users/nelle/Desktop/data-shell`로 되돌아 간다:
...
예를 들어, Nelle의 홈 디렉토리가 `/Users/nelle`이라면, `~/data`은 `/Users/nelle/data`와 동치가 된다.
...
탭을 다시 누르면, Bash가 명령문에 `2012-07-03/`을 추가하는데, 왜냐하면 유일하게 가능한 자동완성조건이기 때문이다.
...
\noindent 1. [절대 경로 vs. 상대 경로] 동일 작업을 수행하는 수많은 방법을 연습문제를 통해 확인해보자. $\text{/home/amanda/data/}$ 디렉토리에서 시작할 때, Amanda가 홈디렉토리인 $\text{/home/amanda}$로 돌아가도록 사용할 수 있는 명령어를 아래에서 선택하시요.

\begin{tasks}[label=(\arabic*)](1)
  \task $\text{cd .}$
  \task $\text{cd /}$
  \task $\text{cd /home/amanda}$
  \task $\text{cd ../..}$
  \task $\text{cd ~}$
  \task $\text{cd home}$
  \task $\text{cd ~/data/..}$
...
\noindent 3. [`ls` 독해 능력] 상기 그림(도전과제 질문에 사용되는 파일 시스템)에 나온 디렉토리 구조를 상정한다.만약 `pwd` 명령어를 쳤을 때 화면에 $\text{/Users/backup}$이 출력되고, `-r` 인자는 `ls` 명령어가 역순으로 화면에 출력하게 한다면, 어떤 명령어가 다음을 화면에 출력할까요?

\begin{tasks}[label=(\arabic*)](1)
  \task $\text{ls pwd}$
  \task $\text{ls -r -F}$
  \task $\text{ls -r -F /Users/backup}$
  \task 위 2 혹은 3, 하지만, 1은 아님.
\end{tasks}

\noindent 2. [상대경로 해결] 만약 `pwd` 명령어를 쳤을 때, 화면에 $\text{/Users/thing}$이 출력된다면, $\text{ls -F ../backup}$은 무엇을 출력할까요?

\begin{tasks}[label=(\arabic*)](1)
  \task $\text{../backup: No such file or directory}$
  \task $\text{2012-12-01 2013-01-08 2013-01-27}$
  \task $\text{2012-12-01/ 2013-01-08/ 2013-01-27/}$
  \task $\text{original/ pnas\_final/ pnas\_sub/}$
\end{tasks}


\end{Exercise}

![연습문제 제시된 파일 시스템](images/filesystem-challenge.svg){fig-align="center" width="474"}
