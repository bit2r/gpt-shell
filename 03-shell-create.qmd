...
방금 `thesis` 디렉터리를 만들었으므로 아직 아무것도 없다:

```bash
$ ls -F thesis
```

`mkdir` 명령어는 한 번에 하나의 디렉터리를 만드는 것으로 국한되지 않는다. `-p` 옵션을 사용하면 `mkdir` 한 번의 작업으로 중첩된 하위 디렉터리가 있는 디렉터리를 만들 수 있습니다:\index{ls!ls -p}
...
`ls` 명령어 `-R` 옵션을 추가하면 디렉터리 내에 중첩된 모든 하위 디렉터리가 나열됩니다. `ls -FR`을 사용하여 `project` 디렉터리에서 방금 만든 디렉터리 계층 구조를 재귀적으로 나열해 보겠습니다:\index{ls!ls -FR}
...
공백이 명령라인 인터페이스에서 인자를 구별하는데 사용되기에, 파일과 디
렉토리 명에서는 피하는 것이 상책이다. 공백 대신에 `-` 혹은 `_` 문자를 사용한
...
![나노 편집기 실제 사용화면](images/nano-screenshot.png){#fig-nano}
...
재귀 옵션 `-r`을 사용하여 디렉터리 및 디렉터리 내부 모든 콘텐츠를 복사할 수도 있다(예를 들어, 디렉터리를 백업하는 경우):\index{ls!ls -r} \index{재귀}

```bash
$ cp -r thesis thesis_backup
```

`thesis`와 `thesis_backup` 디렉터리 내부 결과를 `ls` 명령어로 확인할 수 있다:
...
`-r` 플래그를 포함하는 것이 중요하다. 디렉터리를 복사하려는 경우 이 옵션을 생략하면 `-r not specified` 때문에 디렉터리가 생략되었다는 메시지가 표시된다.
...
셸을 사용하여 삭제한 파일을 복원할 수 있는 방법이 없으므로 `rm -r`은 매우 주의해서 사용해야 한다 (대화형 옵션인 `rm -r -i`를 추가하는 것도 고려해 볼 수 있다).
...
한 번에 여러 파일을 복사하거나 이동해야 하는 경우가 종종 있다. 파일명을 목록으로 제공하거나 와일드카드를 사용하여 패턴을 지정하면 된다. **와일드카드(Wildcards)**는 Unix 파일 시스템을 탐색할 때 특정한 패턴이나 일련의 문자를 대체하거나 대표하는 데 사용되는 특수 문자로 '*', '?', '%' 등이 대표적이다.
...
하나이상의 파일명 다음에 디렉토리명이 주어지게 되면(즉, 목적지 디렉토리는 마지막 인자에 위치해야 한다.), 
...
그리고 최소 한자 이상의 문자로 끝나는 임의의 문자열을 매칭한다고 표현할 수 있는데 
'?'이 한 문자를 매칭해야하고 마지막 \text{*} 은 끝에 임의의 문자숫자와 매칭할 수 있기 때문이다. 
...
다음 명령어 집합 중 어떤 명령어 집합이 상기 목적을 달성할까요? 다른 명령어 집합은 무슨 작업을 수행하는 것일가?

이전 실험이 `2016-05-18`이라는 폴더에 있고, 이 폴더에는 데이터 폴더가 있으며, 이 폴더에는 차례로 데이터 파일이 들어 있는 `raw` 및 `processed`라는 이름의 폴더가 있다고 가정하자. 목표는 `2016-05-18` 폴더의 폴더 구조를 `2016-05-20`이라는 폴더에 복사하여 최종 디렉터리 구조가 다음과 같이 되도록 하는 것이다:
...
처음 두 쉘 명령은 목적을 달성한다. 첫 번째 쉘명령은 상대 경로를 사용하여 하위 디렉터리 앞에 최상위 디렉터리를 만든다.

세 번째 쉘명령은 `mkdir` 기본 동작이 존재하지 않는 디렉터리의 하위 디렉터리를 만들지 않는다. 따라서, 중간에 폴더를 먼저 만들어야 하기 때문에 오류를 발생된다.

네 번째 쉘명령도 목적을 달성한다. 하나 이상의 디렉터리 경로 뒤에 `-p` 옵션을 사용하면 `mkdir` 필요에 따라 중간 하위 디렉터리를 만들게 된다는 점을 기억한다.

마지막 쉘명령은 'data' 디렉터리와 같은 수준에서 'raw' 및 'processed' 디렉터리를 생성한다.
:::
:::
...
> 디렉터리를 "shell-lesson-data/exercise-data/writing/thesis"로 변경하고, E는 "draft.txt"라는 새 파일을 생성한 다음 파일 이름을 "thesis.txt"로 변경한다.
...
> 렉터리를 "shell-lesson-data/exercise-data/writing/thesis"로 변경하고, 해당 디렉터리에 "data.sql", "clean.py", "EDA.R" 및 "report.qmd"라는 4개의 파일을 생성하고, 상위 디렉터리에 "backup"이라는 새 디렉터리를 생성한 후 현재 디렉터리의 모든 파일을 "backup" 디렉터리로 복사한다.
