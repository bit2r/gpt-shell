...
바탕화면(Desktop) `shell-lesson-data` 디렉토리로 돌아가서 
...
다음으로 `exercise-data/writing` 디렉토리로 이동하여 그 안에 무엇이 들어 있는지 살펴봅시다:
...
방금 `thesis` 디렉터리를 만들었으므로 아직 아무것도 없습니다:
...
공백 대신에 `-` 혹은 `_` 문자를 사용한다. (예를 들어, `north pacific gyre/` 대신 `north-pacific-gyre/`을 사용))
...
타이핑한 텍스트가 만족스럽다면, `컨트롤+O` (`Control-O`, `Ctrl` 혹은 콘트롤 키보드를 누르면서 `O` 를 누름)를 눌러서 데이터를 디스크에 쓰면 저장된다:
...
`.png` 확장자는 PNG 이미지 등등을 나타낸다.\index{파일!확장자}
