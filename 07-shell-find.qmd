...
![`Find` 찾기 예제 파일 구조](images/find-file-tree.svg){width=100%}
...
\noindent 1. [다른 특성을 갖는 파일 찾아내기] `find` 명령어에 "test"로 알려진 다른 기준을 제시해서 특정 속성을 갖는 파일을 지정할 수 있다. 예를 들어,
...
\task 힌트: `-type`, `-mtime`, `-user` 플래그 세개를 모두 사용해야 한다.
\task 힌트: `-mtime` 값을 음수를 지정해야 된다 --- 왜일까?
\end{tasks}

\noindent 2. [`find` 파이프라인 독해 능력] 다음 쉘 스크립트에 대해서 어떤 작업을 수행하는지 짧은 설명문을 작성하세요. 


\detokenize{$ wc -l $(find . -name '*.dat') | sort -n}


\noindent 3. [매칭후 빼내기] `grep` 명령어의 `-v` 옵션은 패턴 매칭을 반전시킨다. 패턴과 매칭하지 *않는* 행만 출력시킨다. 다음 명령어 중에서 어느 것이 $\detokenize{/data}$ 폴더에 `s.txt`로 끝나는 (예로, `animals.txt` 혹은 `planets.txt`), 
하지만 `net` 단어는 포함하지 *않게* 모든 파일을 찾아낼까요? 정답을 생각해냈다면, `data-shell` 디렉토리에서 다음 명령어를 시도해본다.

\begin{tasks}[label=(\arabic*)](1)
 \task \detokenize{`find data -name '*s.txt' | grep -v net`}
 \task \detokenize{`find data -name *s.txt | grep -v net`}
 \task \detokenize{`grep -v "temp" $(find data -name '*s.txt')`}
 \task   위의 항목에 해당사항 없음.
