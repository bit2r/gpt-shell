...
[Jeff Rothenberg가 말했듯이](http://www.clir.org/pubs/archives/ensuring.pdf), "디지털 정보는 어느 것이 먼저 오든 영원한 영속성을 가지거나 혹은 5년이다." 운이 좋은 경우 인기 콘텐트는 종종 [백업된다](http://wiki.c2.com/?ComputerErrorHaiku).
:::
...
매칭을 "The" 단어 자체만 포함하는 행만 매칭시키려면, `grep`명령어에 `-w` 옵션을 주게 되면, 단어 경계로 매칭을 제한시킨다.
...
따라서, 올바른 결과를 도출해야 하고, 우아하고(elegance), 가독성이 좋고(readability), 속도를 다 함께 고려하여 선택한다.
...
다시, `find` 명령어는 정말 옵션이 많다; 가장 간단한 것이 어떻게 동작하는지 시연하기 위해, 다음과 같은 디렉토리 구조를 사용할 것이다.

![`Find` 찾기 예제 파일 구조](images/find-file-tree.svg){fig-align="center" width="77%"}
